...
namespace GLT {
	...
	public:
		...
		FORCEINLINE const glm::mat4& get_view() const								{ return m_view_matrix; }
		DEFAULT_GETTER(f32, 														clipping_near)
		DEFAULT_GETTER(f32, 														clipping_far)
		DEFAULT_GETTERS(f32, 														perspective_fov_y)
		DEFAULT_GETTERS(glm::vec3, 													direction)
		DEFAULT_GETTERS(glm::vec3, 													position)

		FORCEINLINE void force_set_projection_matrix(const glm::mat4& projection)	{ m_projection_matrix = projection; }
		FORCEINLINE void force_set_view_matrix(const glm::mat4& view)				{ m_view_matrix = view; }

		// MAYBE: change to directly construct inverse matrix
		glm::mat4 get_inverse_projection(const f32 aspect_ratio) { return glm::inverse(glm::perspective(glm::radians(m_perspective_fov_y), aspect_ratio, m_clipping_near, m_clipping_far)); }
		FORCEINLINE glm::mat4 get_inverse_projection() const { return glm::inverse(m_projection_matrix); }
		FORCEINLINE glm::mat4 get_inverse_view() const { return glm::inverse(m_view_matrix); }

		void set_clipping_dist(const f32 near_dis, const f32 far_dis);
