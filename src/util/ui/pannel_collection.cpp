					...
					// Foreground colors
						...
						if (i < params.size()) {
							...
							if (type == 5 && i < params.size())
								...
								const auto r = params[i++] / 255.0f;
								const auto g = params[i++] / 255.0f;
								const auto b = params[i++] / 255.0f;
...
#define GRAY(value)		IM_COL32(value, value, value, 255);
	...
	void custom_frame(const f32 width_left_side, std::function<void()> left_side, std::function<void()> right_side) {
		...
		if (ImGui::BeginTable("test_master_divider", 2, flags)) {
			...
			ImGui::GetWindowDrawList()->AddRectFilled(uperleft_corner, lowerright_corner, main_color /*GLT::UI::convert_color_to_int(color)*/);
